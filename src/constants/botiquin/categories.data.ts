// src/constants/botiquin/categories.data.ts
...
// ... importa otras ilustraciones ...

export interface BotiquinCategory {
  id: string; // ej: 'ansiedad', 'tristeza' (debe coincidir con las keys en allTechniquesData y las rutas)
  title: string; // ej: 'Ansiedad / Angustia'
  illustrationSrc?: string; // Ruta a la imagen (opcional)
  illustrationComponent?: React.FC<React.SVGProps<SVGSVGElement>>; // Componente SVG (opcional)
  accentColorClass: string; // Clase de Tailwind para el borde/acento
  backgroundColorClass?: string; // Clase de Tailwind para el fondo (opcional)
  activitiesCount: number; // Número de actividades a mostrar
  textColorClass?: string; // Clase de Tailwind para el texto (opcional)
  titleHoverColorClass?: string; // Clase de Tailwind para el texto (opcional)
  description?: string; // Opcional, si quieres mostrar una breve descripción en la tarjeta
}

export const botiquinCategories: BotiquinCategory[] = [
  {
    id: 'ansiedad',
